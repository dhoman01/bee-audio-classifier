...
class ANN(object):
    ...
    def __init__(self, N=256, num_hidden=3, num_classes=3):
        self.N = N
    ...
    def build(self):
        ...
        with tf.variable_scope("model", initializer=tf.random_uniform_initializer()) as scope:
            ...
            last_layer = self._layer(self.x, self._weight_variables([N,N], 'input_layer'), self._bias_variable([N], 'input_bias'))

            # Build the hidden layers
            for i in range(self.num_hidden):
                last_layer = self._layer(last_layer, self._weight_variables([N,N], 'hidden_layer_' + str(i)), self._bias_variable([N], 'hidden_bias_' + str(i)))
        
            # Build the logits of the ANN
            self.logits = self._layer(last_layer, self._weight_variables([N,self.num_classes], 'output_layer'), self._bias_variable([self.num_classes], 'output_bias'))
        ...
        with tf.variable_scope("training", initializer=tf.random_uniform_initializer()) as scope:
            ...
            self.train_op = tf.train.AdamOptimizer(1e-4).minimize(cross_entropy)
